...
import { ShoppingCart, CheckCircle, Clock, TrendingUp, Plus, AlertCircle } from "lucide-react";
...
export default function SellerDashboard() {
  ...
  const orders  = getSellerOrders(user.sellerId);
    ...
    <div className="flex flex-col min-h-full bg-[#F8FAFC]">
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          {/* Wallet hero */}
          <div className="lg:col-span-2 rounded-xl p-6 text-white flex flex-col justify-between"
        ...
        {frozen.length > 0 && (
          <div className="space-y-2">
            {frozen.map((o) => (
              <div key={o.orderId} className="flex items-center gap-3 p-4 rounded-xl bg-[#FEF2F2] border border-[#FECACA]">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[#DC2626] text-xs mt-0.5">
                    {o.orderId} · {o.productTitle} · يحتاج <strong>{formatMAD(o.totalCost)}</strong>
                  </p>
                </div>
        ...
        <Card>
          <CardHeader>
            ...
            <Link href="/seller/orders" className="text-xs font-semibold hover:underline" style={{ color: "#4361EE" }}>عرض الكل ←</Link>
          </CardHeader>
          {recent.length === 0 ? (
            <p className="text-center text-[#94A3B8] py-8 text-sm">لا توجد طلبات بعد — أضف طلبك الأول!</p>
          ) : (
                ...
                <thead>
                  <tr className="border-b border-[#F1F5F9]">
                    {["رقم الطلب", "العميل", "المدينة", "المنتج", "الحالة", "التاريخ"].map((h) => (
                      <th key={h} className="pb-3 text-right text-xs font-semibold text-[#94A3B8] px-3 first:pr-0">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#F8FAFC]">
                  {recent.map((o) => (
                    <tr key={o.orderId} className="hover:bg-[#F8FAFC] transition-colors">
                      <td className="py-3 px-3 first:pr-0 font-mono font-bold text-xs" style={{ color: "#4361EE" }}>{o.orderId}</td>
