...
export default function ProductsPage() {
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#F8FAFC]">
      <Header title="المنتجات المتاحة" />

      <div className="flex-1 p-6 space-y-5">
        <div>
          <h2 className="text-xl font-bold text-[#1E293B]">كتالوج المنتجات</h2>
          <p className="text-[#64748B] text-sm">{active.length} منتج متاح للبيع</p>
        </div>
          ...
          {active.map((p) => (
            <Card key={p.productId} className="flex flex-col gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0]">
                <div>
                  <p className="text-xs text-[#94A3B8]">تكلفة التوريد</p>
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${
                  ...
                  {p.stockQuantity === 0 ? "نفد المخزون" : `${p.stockQuantity} قطعة`}
                </span>
                <span className="text-xs text-[#64748B]">
                  ربح: <span className="font-bold text-[#16A34A]">{formatMAD(p.sellingPrice - p.sourcingPrice)}</span>
                </span>
              </div>
            </Card>
          ))}
